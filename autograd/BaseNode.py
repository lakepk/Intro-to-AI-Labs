...
class Linear(Node):
    ...
    def backcal(self, grad):
        ...
        self.grad.append(X_reshape.T @ grad_reshape)
        self.grad.append(grad_reshape.sum(axis=0))
...
class LogSoftmax(Node):
    ...
    def cal(self, X):
        ...
        X_stable = X - X_max
        expX = np.exp(X_stable)
        sumexp = expX.sum(axis=self.dim, keepdims=True)
        self.cache.append(expX / sumexp)
        return X_stable - np.log(sumexp)
...
class CrossEntropyLoss(Node):
    ...
    def cal(self, X):
        ...
        self.cache.append(X)
        return -np.sum(
            np.log(np.take_along_axis(X, np.expand_dims(y, axis=-1), axis=-1) + 1e-8))
    ...
    def backcal(self, grad):
        ...
        np.put_along_axis(ret, np.expand_dims(y, axis=-1), -1.0 / (probs + 1e-8), axis=-1)
        return grad * ret